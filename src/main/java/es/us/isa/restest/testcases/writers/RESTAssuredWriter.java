...
/**
 * This class defines a test writer for the REST Assured framework. It creates a
 * Java class with JUnit test cases
...
public class RESTAssuredWriter implements IWriter {
    ...
    private boolean logToFile; // If 'true', REST-Assured requests and responses will be logged into external
                               // files
    ...
    public RESTAssuredWriter(String specPath, String testConfPath, String targetDirJava, String className,
            String packageName, String baseURI, Boolean logToFile) {
    ...
    public void write(Collection<TestCase> testCases) {
        ...
        for (TestCase t : testCases) {
            // System.out.println("");
            logger.info("Test case WRITING {}", t);
         ...
         * compiler.run(System.in, System.out, System.err, TEST_LOCATION +
         * this.specification.getInfo().getTitle().replaceAll(" ", "") + "Test.java");
         */
    }
    ...
    private String generateImports(String packageName) {
        ...
        // content += "import
        // es.us.isa.restest.testcases.restassured.filters.CoverageFilter;\n";
    ...
    private String generateAttributes(String specPath) {
        ...
        if (statefulFilter)
            content += "\tprivate static final StatefulFilter statefulFilter = new StatefulFilter(\""
                    + specPath.substring(0, specPath.lastIndexOf('/')) + "\");\n";
        ...
        // if (statefulFilter) {
        // content += "\tprivate static final ObjectMapper objectMapper = new
        // ObjectMapper();\n";
        // }
    ...
    private String generateTest(TestCase t, int instance) {
        ...
        // Log response time of the test case
        content += generateResponseTimeLogger();
    ...
    private String generateResponseTimeLogger() {
        return "\t\t\tSystem.out.println(\"Response Time: \" + response.getTime() + \" ms\");\n";
    }
    ...
    private String generateFiltersInitialization(TestCase t) {
        ...
        content += "\t\tnominalOrFaultyTestCaseFilter.updateFaultyData(" + t.getFaulty() + ", "
                + t.getFulfillsDependencies() + ", \"" + escapeJava(t.getFaultyReason()) + "\");\n" +
                "\t\tstatusCode5XXFilter.updateFaultyData(" + t.getFaulty() + ", " + t.getFulfillsDependencies()
                + ", \"" + escapeJava(t.getFaultyReason()) + "\");\n";
    ...
    private String generatePathParameters(TestCase t) {
        ...
        for (Entry<String, String> param : t.getPathParameters().entrySet())
            content += "\t\t\t\t.pathParam(\"" + param.getKey() + "\", \""
                    + escapeJava(param.getValue().replace("{", "")) + "\")\n";
        // TODO: Once REST-Assured fixes the bug, stop removing "{" chars from path
        // parameters
    ...
    private String generateFormParameters(TestCase t) {
        ...
        for (Entry<String, String> param : t.getFormParameters().entrySet()) {
            content += checkIfExists(param.getValue())
                    ? "\t\t\t\t.multiPart(\"" + param.getKey() + "\", new File(\"" + escapeJava(param.getValue())
                            + "\"))\n"
    ...
    // }
    // // If there is a default response, use it if the expected status code is not
    // found
    ...
    // }
    //
    // // Assert status code only if it was found among possible status codes.
    // Otherwise, only JSON structure will be validated
    ...
    //// + ");\n\n";
    //// }
    //// content = "\t\t\tassertTrue(\"Received status 500. Server error found.\",
    ...
    // if (!hasDefaultCase)
    // content += "\t\tdefault: \n"
    // + "\t\t\tSystem.err.println(\"Unexpected HTTP code: \" +
