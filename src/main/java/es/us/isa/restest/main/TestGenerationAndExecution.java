...
import es.us.isa.restest.generators.TimeDrivenConstraintBasedTestCaseGenerator;
...
public class TestGenerationAndExecution {
    ...
    private static String propertiesFilePath = "src/test/resources/restfulWebApi/restfulWebApi.properties";
    ...
    private static Integer numTestCasesPerIteration; // Number of test cases per operation
    ...
    // ARTE
    private static Boolean learnRegex; // Set to 'true' if you want RESTest to automatically generate Regular
                                       // expressions that filter the semantically generated input data
    ...
    public static void main(String[] args) throws RESTestException {
        ...
        while (totalNumTestCases == -1 || runner.getNumTestCases() < totalNumTestCases) {
            ...
            logger.info("Total test cases split by API endpoints: {}", runner.getNumTestCasesPerOperation());
            iteration++;
        }
    ...
    private static AbstractTestCaseGenerator createGenerator() throws RESTestException {
        ...
        logger.info("CONF 1: {} {} {}", confPath, OAISpecPath, spec);
        conf = loadConfiguration(confPath, spec);
        logger.info("CONF 2: {}", conf);
        ...
        switch (generator) {
            ...
            case "CBT":
                ...
                break;
            case "TDCBT":
                gen = new TimeDrivenConstraintBasedTestCaseGenerator(spec, conf, numTestCasesPerIteration);
                ((TimeDrivenConstraintBasedTestCaseGenerator) gen).setFaultyDependencyRatio(faultyDependencyRatio);
                ((TimeDrivenConstraintBasedTestCaseGenerator) gen).setInputDataMaxValues(inputDataMaxValues);
                ((TimeDrivenConstraintBasedTestCaseGenerator) gen).setReloadInputDataEvery(reloadInputDataEvery);
                gen.setFaultyRatio(faultyRatio);
                break;
            case "ART":
    ...
    private static void readParameterValues() {
        ...
        if (readParameterValue("testsperiteration") != null)
            numTestCasesPerIteration = Integer.parseInt(readParameterValue("testsperiteration"));
        logger.info("Number of test cases per iteration: {}", numTestCasesPerIteration);
        ...
        if (readParameterValue("maxNumberOfTriesToGenerateRegularExpression") != null)
            maxNumberOfTriesToGenerateRegularExpression = Integer
                    .parseInt(readParameterValue("maxNumberOfTriesToGenerateRegularExpression"));
        logger.info("Maximum number of tries to generate a regular expression: {}",
                maxNumberOfTriesToGenerateRegularExpression);
