...
/**
 * This class implements a basic test workflow: test generation -&gt; test
 * writing -&gt; class compilation and loading -&gt; test execution -&gt; test
 * report generation -&gt; test coverage report generation
 * 
...
public class RESTestRunner {
    ...
    private long baselineResponseTime = 0; // The baseline response time based on which test case generation varies
    private HashMap<String, Long> averageResponseTimePerAPIEndpoint = new HashMap<>();
    private boolean learnRegex;
    private boolean secondPredicateSearch;
    private OpenAPISpecification spec;
    private String confPath;
    private HashMap<String, Long> numTestCasesPerOperation = new HashMap<>();

    private static final Logger logger = LogManager.getLogger(RESTestRunner.class.getName());

    public RESTestRunner(String testClassName, String targetDir, String packageName, Boolean learnRegex,
            Boolean secondPredicateSearch, OpenAPISpecification spec, String confPath,
            AbstractTestCaseGenerator generator, IWriter writer, AllureReportManager reportManager,
    ...
    public void run() throws RESTestException {
        ...
        testGeneration(); // link this generation to previous testExecution by somehow getting
    ...
    protected void updateResponseTimes() {
        generator.setBaselineResponseTime(this.baselineResponseTime);
        generator.setAverageResponseTimePerAPIEndpoint(this.averageResponseTimePerAPIEndpoint);
    }
    ...
    private void testGeneration() throws RESTestException {
        ...
        for (TestCase testcase : testCases) {
            this.numTestCasesPerOperation.putIfAbsent(testcase.getOperationId(), (long) 0);
            this.numTestCasesPerOperation.put(testcase.getOperationId(),
                    this.numTestCasesPerOperation.get(testcase.getOperationId()) + 1);
        }
        ...
        // Write test cases
        String filePath = targetDir + "/" + testClassName + ".java";
        logger.info("Writing {} test cases to test class {}", testCases.size(), filePath); // modify testCases to be
                                                                                           // written by reading
                                                                                           // response time of prev run
    ...
    protected void testExecution(Class<?> testClass) {
        ...
        logger.info("{}", result);
        logger.info("THE DICT IS {}", timer.getDuration());
        logger.info("THE AV DICT IS {}", timer.getAverageDurationsPerAPIEndpoint());
        logger.info("THE AV RESP IS {}", timer.getAverageResponseTime());
        this.averageResponseTimePerAPIEndpoint = timer.getAverageDurationsPerAPIEndpoint();
        this.baselineResponseTime = timer.getAverageResponseTime();
        // generator.setBaselineResponseTime(this.baselineResponseTime);
        // generator.setAverageResponseTimePerAPIEndpoint(this.averageResponseTimePerAPIEndpoint);
        // Map<String, Object> results = listener.getResults();
        // for (Map.Entry<String, Object> entry : results.entrySet()) {
        // String testName = entry.getKey();
        // Object testResult = entry.getValue();
        // System.out.println("YO" + testName + " returned " + testResult);
        // }
    ...
    public HashMap<String, Long> getNumTestCasesPerOperation() {
        return numTestCasesPerOperation;
    }
