...
public abstract class AbstractTestCaseGenerator {
    ...
    protected Float faultyRatio = 0f; // Ratio (0-1) of faulty test cases to be generated on each operation. Defaults
                                      // to 0.1
    protected int numberOfTests; // Number of test cases to be generated for each operation // THIS HAS TO VARY
                                 // BASED ON OPEARTION RESPONSE TIME
    ...
    long baselineResponseTime;
    HashMap<String, Long> averageResponseTimePerAPIEndpoint;
    ...
    private void preconditions(TestConfigurationObject conf) {
        ...
        for (Operation testOperation : conf.getTestConfiguration().getOperations()) {
            List<ParameterFeatures> requiredParams = SpecificationVisitor
            ...
            if (testOperation.getTestParameters() != null) {
                ...
                for (TestParameter testParameter : testOperation.getTestParameters()) {
                    ...
                    ParameterFeatures param = SpecificationVisitor.findParameterFeatures(
    ...
    public Collection<TestCase> generate(Collection<TestConfigurationFilter> filters) throws RESTestException {
        ...
        for (TestConfigurationFilter filter : filters) {
            ...
            for (HttpMethod method : filter.getMethods()) {
                ...
                logger.info("FILTER METH {}", filter.getMethods());
                testCases.addAll(generate(filter.getPath(), method));
            }
        }
        logger.info("TEst cases are {}", testCases);
        return testCases;
    }
    ...
    public Collection<TestCase> generate() throws RESTestException {
        ...
        for (Operation testOperation : conf.getTestConfiguration().getOperations()) {
            logger.info("HELLO!! {} {} {}", testOperation.getMethod(), testOperation.getTestPath(),
                    testOperation.getOperationId());
     ...
     * There's an exception: if stateful generators are configured, or ARTE is
     * enabled, we cannot assure that the test case
            ...
            for (TestParameter param : testParameters) {
                ...
                for (Generator g : param.getGenerators()) {
                    ...
                    if (gen instanceof ParameterGenerator) {
                        ...
                        ((ParameterGenerator) gen).setParameterType(SpecificationVisitor
