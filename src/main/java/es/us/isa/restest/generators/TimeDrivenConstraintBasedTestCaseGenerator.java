...
/**
 * This class implements a constraint-based test case generator using
 * IDLReasoner, a CSP-based tool for the automated analysis of inter-parameter
 * dependencies
 * 
 * @author Alberto Martin-Lopez
 *
 */
public class TimeDrivenConstraintBasedTestCaseGenerator extends AbstractTestCaseGenerator {

    public static final String INTER_PARAMETER_DEPENDENCY = "inter_parameter_dependency";

    private Float faultyDependencyRatio = 0.5f; // Ratio of faulty test cases due to inter-parameter deps. Defaults to
                                                // 0.5
    ...
    // Indexes
    private int maxFaultyTestDueToDependencyViolations; // Maximum number of faulty test cases due to dependency
                                                        // violations to be generated
    private int maxFaultyTestsDueToIndividualConstraints; // Maximum number of faulty test cases due to individual
                                                          // constraints to be generated
    int nFaultyTestDueToDependencyViolations; // Current number of faulty test cases due to dependency violations to be
                                              // generated
    int nFaultyTestsDueToIndividualConstraint; // Current number of faulty test cases due to individual constraints to
                                               // be generated

    private static Logger logger = LogManager.getLogger(ConstraintBasedTestCaseGenerator.class.getName());

    public TimeDrivenConstraintBasedTestCaseGenerator(OpenAPISpecification spec, TestConfigurationObject conf,
            int nTests) {
    ...
    /**
     * Set IDLReasoner for the generation of dependency-aware valid/invalid test
     * cases
     * 
    ...
    public void setUpIDLReasoner(Operation testOperation) {
        idlReasoner = null;
        if (hasDependencies(testOperation.getOpenApiOperation())) // If the operation contains dependencies, create new
                                                                  // IDLReasoner for that operation
        {
            try {
                logger.info("OAS ANALYZING {} {} {}", spec.getPath(), testOperation.getTestPath(),
                        testOperation.getMethod());
                idlReasoner = new OASAnalyzer(spec.getPath(), testOperation.getTestPath(), testOperation.getMethod());
            } catch (IDLException e) {
                logger.warn("There was an error processing the dependencies of the operation {} {}: {}",
                        testOperation.getMethod(), testOperation.getTestPath(), e.getMessage());
            }
        }
    }

    @Override
    public void setBaselineResponseTime(long baselineResponseTime) {
        this.baselineResponseTime = baselineResponseTime;
    };

    /* Set baseline Response Time */
    @Override
    public void setAverageResponseTimePerAPIEndpoint(HashMap<String, Long> averageResponseTimePerAPIEndpoint) {
        this.averageResponseTimePerAPIEndpoint = averageResponseTimePerAPIEndpoint;
    };

    /**
     * Refresh the test data used for the generation of test cases. Test data must
     * be fed to IDLReasoner, which in turn uses it for the generation of test cases
     * using a CSP solver.
     * 
     * @param testOperation API operation under test
     */
    public void checkIDLReasonerData(Operation testOperation) {
        if (idlReasoner != null && nTests % reloadInputDataEvery == 0) {
            Map<String, List<String>> inputData = generateInputData(testOperation.getTestParameters()); // Update input
                                                                                                        // data
            try {
    ...
    protected Collection<TestCase> generateOperationTestCases(Operation testOperation) throws RESTestException {
        ...
        logger.info("IN TEST OP GEN {}", testOperation.getOperationId());
        ...
        maxFaultyTestsDueToIndividualConstraints = (int) (((numberOfTests * faultyRatio)
                - maxFaultyTestDueToDependencyViolations));

        nFaultyTestDueToDependencyViolations = 0;
        nFaultyTestsDueToIndividualConstraint = 0;

        while (hasNext(testOperation.getOperationId())) {
    ...
    public TestCase generateNextTestCase(Operation testOperation) throws RESTestException {
        ...
        if (nFaultyTestDueToDependencyViolations < maxFaultyTestDueToDependencyViolations) // Try generating a faulty
                                                                                           // test case violating one or
                                                                                           // more inter-parameter
                                                                                           // dependency
            test = generateNextTestCase(testOperation, INTER_PARAMETER_DEPENDENCY);

        else if (nFaultyTestsDueToIndividualConstraint < maxFaultyTestsDueToIndividualConstraints) // Try generating a
                                                                                                   // faulty test case
                                                                                                   // violating an
                                                                                                   // individual
                                                                                                   // constraint
     ...
     * 
     * @param testOperation API operation under test
     * @param faultyReason  Faulty reason. Possible values: "none",
     *                      "individual_parameter_constraint", and
    ...
    /*
     * Returns a valid test case satisfying all the individual constraints and
     * inter-parameter dependencies described in the API specification
     */
    TestCase generateValidTestCase(Operation testOperation) throws RESTestException {
        ...
        if (idlReasoner != null) { // The operation has inter-parameter dependencies
            ...
            try {
                idl2restestTestCase(test, idlReasoner.getRandomValidRequest(), testOperation); // Generate valid test
                                                                                               // case with IDLReasoner
            } catch (IDLException e) {
            ...
            test = generateRandomValidTestCase(testOperation); // Generate valid test case normally (no need to manage
                                                               // deps.)
    ...
    /*
     * Returns a faulty test case violating one ore more inter-parameter dependency
     * constraints
     */
    TestCase generateFaultyTestCaseDueToViolatedDependencies(Operation testOperation) throws RESTestException {
        ...
        if (idlReasoner != null) { // The operation has inter-parameter dependencies
            test = createTestCaseTemplate(testOperation);
            try {
                idl2restestTestCase(test, idlReasoner.getRandomInvalidRequest(), testOperation); // Generate invalid
                                                                                                 // test case with
                                                                                                 // IDLReasoner
            } catch (IDLException e) {
        ...
        for (TestParameter parameter : testParameters) {
            if (parameter.getWeight() == null || parameter.getWeight() > 0) {
                ...
                generator = getRandomGenerator(
                        nominalGenerators.get(Pair.with(parameter.getName(), parameter.getIn())));
                if (generator instanceof RandomInputValueIterator
                        && ((RandomInputValueIterator) generator).getMaxValues() == 1) {
    ...
    protected boolean hasNext(String OperationId) {
        ...
        if (this.averageResponseTimePerAPIEndpoint != null) {
            ...
            if (sum != 0) {
                frac = this.averageResponseTimePerAPIEndpoint.get(OperationId).doubleValue() / sum;
            }
        }
        logger.info("CHECKING IF OP ID {} HAS NTESTS {} LESS THAN FRACTESTS {} (FRAC {}) ", OperationId, nTests,
                Math.round(frac * numberOfTests), frac);
        return nTests < Math.max(Math.round(frac * numberOfTests), 1);
    }
